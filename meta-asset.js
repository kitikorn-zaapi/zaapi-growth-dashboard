...
function signalClass(r) {
  if (n(r.hook_rate) < 15 || n(r.fti_day7) === 0) return "border-red-500";
  if (n(r.hook_rate) > 35 && n(r.fti) >= 2) return "border-emerald-500";
  if (n(r.frequency) > 3 && n(r.fti_wow) === 0) return "border-yellow-500";
...
function render() {
  ...
  const filtered = rows.filter((r) => funnel === "All" || String(r.funnel).toUpperCase() === funnel);
  ...
  document.getElementById("cards").innerHTML = filtered.map((r) => `
    <article class="bg-slate-900 border ${signalClass(r)} rounded p-3 space-y-1">
      <div class="flex justify-between"><b>${r.ad_code}</b><span class="text-xs px-2 py-0.5 rounded ${String(r.status).toLowerCase()==="kill"?"bg-red-900 text-red-200":"bg-emerald-900 text-emerald-200"}">${r.status || "Live"}</span></div>
      <div class="text-xs text-slate-400">${r.region} · ${r.prod_type} · ${r.angle}</div>
      <div class="text-sm">Hook Rate: ${n(r.hook_rate)}% ${n(r.hook_rate_wow)>=0?"↑":"↓"}</div>
      <div class="text-sm">Thumb-Stop: ${n(r.thumb_stop)} · ThruPlay: ${n(r.thruplay)} · Frequency: ${n(r.frequency)}</div>
      <div class="text-sm">FTI: ${n(r.fti)} ${n(r.fti_wow)>=0?"↑":"↓"} · CPA: ${money(r.cpa_thb, ccy)}</div>
    </article>`).join("");
  ...
  const body = sorted.map((r) => `<tr><td class="px-2 py-1 border-b border-slate-800">${r.ad_code}</td><td class="px-2 py-1 border-b border-slate-800">${n(r.hook_rate)}%</td><td class="px-2 py-1 border-b border-slate-800">${n(r.fti)}</td><td class="px-2 py-1 border-b border-slate-800">${money(r.cpa_thb, ccy)}</td></tr>`).join("");
