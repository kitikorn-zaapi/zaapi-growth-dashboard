...
//             raw_meta_daily (Meta spend), action_log, learning_accum
...
function f(row, keys, fallback = '') { return ZaapiDataService.pick(row, keys, fallback); }
function fmtTHB(n) { return `฿${ZaapiDataService.toNumber(n).toLocaleString(undefined, { maximumFractionDigits: 0 })}`; }

function latestCWFromRows(rows) {
  const cws = [...new Set(rows.map(r => ZaapiDataService.fmtCW(f(r, ['CW', 'cw']))))].filter(Boolean);
  if (!cws.length) return '';
  return cws.sort((a, b) => parseInt(a.replace(/\D/g, ''), 10) - parseInt(b.replace(/\D/g, ''), 10)).slice(-1)[0];
}

function aggGoogleDailyToLatestCW(rows) {
  if (!rows.length) return { cw: '', campaigns: [] };
  const latestCW = latestCWFromRows(rows);
  const byCampaign = new Map();
  rows.filter(r => ZaapiDataService.fmtCW(f(r, ['CW', 'cw'])) === latestCW).forEach(r => {
...
async function initSearch() {
  ...
  try {
    const tabs = await ZaapiDataService.fetchTabs([
      'raw_google_daily', 'raw_google_weekly_is', 'raw_meta_daily', 'action_log',
    ]);
    const learningAccum = await ZaapiDataService.fetchTab('learning_accum').catch(() => []);
    ...
    const gRows = g.campaigns.sort((a, b) => b.spend - a.spend).map(c => {
      ...
      return `<tr>${td(c.campaign_name)}${td(c.region)}${td(c.country)}${td(c.channel_type)}${td(fmtTHB(c.spend))}${td(c.clicks.toLocaleString())}${td(c.impressions.toLocaleString())}${td(isCell)}${td(lostRankCell, rankColor(is.lost_IS_rank))}${td(lostBudCell)}${td(g.cw)}</tr>`;
    }).join('');
    document.getElementById('google-table').innerHTML = gHead + (gRows || `<tr><td class="px-2 py-3 text-slate-500" colspan="11">No data for latest CW</td></tr>`);
    ...
    const mRows = m.campaigns.sort((a, b) => b.spend - a.spend).map(c =>
      `<tr>${td(c.campaign_name)}${td(c.region)}${td(c.country)}${td(c.funnel)}${td(c.status)}${td(fmtTHB(c.spend))}${td(c.impressions.toLocaleString())}${td(c.clicks.toLocaleString())}${td(c.reach.toLocaleString())}${td(m.cw)}</tr>`
    ).join('');
    document.getElementById('meta-table').innerHTML = mHead + (mRows || `<tr><td class="px-2 py-3 text-slate-500" colspan="10">No Meta data for latest CW</td></tr>`);

    // ═══ AI Suggestions Panel (campaign + overview layers) ═══
    const latestCW = g.cw || m.cw || latestCWFromRows(tabs.action_log || []);
    ...
    if (panelEl && window.AISuggestionsPanel) {
      window.AISuggestionsPanel.render(panelEl, {
        actionLog: tabs.action_log || [],
        learningAccum,
        currentCW: latestCW,
        layers: ['campaign', 'overview'],  // campaign layer + overview (overview often affects campaigns)
        region: null,
        lookbackWeeks: 2,
        title: '🤖 AI Suggestions · Campaign Layer + Last 2 Weeks Results',
      });
    }
